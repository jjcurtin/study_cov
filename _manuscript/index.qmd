...
x <- rep(c(0, 1), each = n_obs / 2) # <2>


sigma <- diag(n_covs + 1) # <3>
n_good_covs <- n_covs * p_good_covs  
corr_matrix <- matrix(.3, 
                      nrow = n_good_covs, 
                      ncol = n_good_covs)
diag(corr_matrix) <- 1
...
ycovs <- MASS::mvrnorm(n_obs, mu = rep(0, n_covs + 1), Sigma = sigma) # <4>
y <- ycovs[, 1]
covs <- ycovs[, -1]
...
df <- tibble::tibble(y = y, x = x) |> 
   dplyr::bind_cols(covs)
```

1. Set research context parameters.
2. Create dichotomous focal predictor (X).
3. Create variance/covariance matrix. Good predictors have 0.3 correlation with each other.
4. Generate Y and all covariates using a multivariate normal distribution.
5. Add in X effect on Y.
6. Combine into final data tibble.
