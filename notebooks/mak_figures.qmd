...
d_full <- read_csv(here::here(path_processed, "results.csv"),
              show_col_types = FALSE)
...
d_full <- d_full |> 
  mutate(method = factor(method, 
                         ...
                         labels = c("No covariates", "All covariates", "p-hacking", 
                                    "Separate lm w/o X", 
                                    "Separate lm with X",
                                    "Simultaneous lm w/o X", 
                                    "Simultaneous lm with X", 
                                    "Simultaneous LASSO w/o X",
                                    "Simultaneous LASSO with X")))
...
method_colors <- c("No covariates" = "#FF7D00",
                   ...
                   "p-hacking" = "#C92C04",
                   "Separate lm with X" = "#5489BA",
                   "Separate lm w/o X" = "#5489BA",
                   "Simultaneous lm with X" = "#664896",
                   "Simultaneous lm w/o X" = "#664896",
                   "Simultaneous LASSO with X" = "#4A7C59",
                   "Simultaneous LASSO w/o X" = "#4A7C59")


method_linetypes <- c("No covariates" = "solid",
                     "All covariates" = "solid",
                     "p-hacking" = "solid",
                     "Separate lm w/o X" = "dashed",
                     "Separate lm with X" = "solid",
                     "Simultaneous lm w/o X" = "dashed",
                     "Simultaneous lm with X" = "solid",
                     "Simultaneous LASSO w/o X" = "dashed",
                     "Simultaneous LASSO with X" = "solid")

method_linewidths <- c("No covariates" = .75,
                     "All covariates" = .75,
                     "p-hacking" = .5,
                     "Separate lm w/o X" = .5,
                     "Separate lm with X" = .5,
                     "Simultaneous lm w/o X" = .5,
                     "Simultaneous lm with X" = .5,
                     "Simultaneous LASSO w/o X" = .5,
                     "Simultaneous LASSO with X" = .5)

method_patterns <- c("No covariates" = "none",
                     "All covariates" = "none",
                     "p-hacking" = "none",
                     "Separate lm w/o X" = "stripe",
                     "Separate lm with X" = "none",
                     "Simultaneous lm w/o X" = "stripe",
                     "Simultaneous lm with X" = "none",
                     "Simultaneous LASSO w/o X" = "stripe",
                     "Simultaneous LASSO with X" = "none")
```
...
type_I_summary <- d_0 |>
  ...
  mutate(group = case_match(method,
                            ...
                            "Separate lm w/o X" ~ "Separate lm",
                            "Separate lm with X" ~ "Separate lm",
                            "Simultaneous lm w/o X" ~ "Simultaneous lm",
                            "Simultaneous lm with X" ~ "Simultaneous lm",
                            "Simultaneous LASSO w/o X" ~ "Simultaneous LASSO",
                            "Simultaneous LASSO with X" ~ "Simultaneous LASSO"))
...
```{r}
#| label: fig-bar-1
#| fig-cap: "Type I Error by Method. Average type I error rate across all research contexts is displayed for each method separately by color. Traditional selection methods are on the left, data-driven selection methods are on the right. Striped shading indicates data-driven selection methods that do not control for X."
#| fig-width: 8.75
...
segment_labels <- tibble::tibble(
  x = c(mean(c(0.5, 2.6)),
        ...
        mean(c(6.2, 8.0)),
        mean(c(8.4, 10.2))),
  y = c(.13, .13, .08, .08, .08) + 0.015,   
  label = c("Traditional selection", "Data-driven selection", "Separate lm", "Simultaneous lm", "Simultaneous LASSO")
)
...
ggplot(
  ...
  geom_segment(aes(y = .13, x = .5, xend = 2.6)) +
  geom_segment(aes(y = .13, x = 2.8, xend = 10.2)) +
...
observations <- d_0 |> 
  ...
  scale_y_continuous(breaks = c(0.05, 0.07, 0.09, 0.11, 0.13),
                     limits = c(0.045, .13)) +
  labs(x = NULL,
...
ycor <- d_0 |> 
  ...
  scale_y_continuous(breaks = c(0.05, 0.07, 0.09, 0.11, 0.13, 0.15), 
                     limits = c(.045, .15)) +
  labs(y = NULL,
       x = NULL,
...
observations_2 <- d_02 |>
  ...
  geom_line() +
  scale_x_continuous(breaks = c(50, 100, 150, 200, 300, 400)) +
  scale_y_continuous(breaks = c(0.10, .30,  .50,  .70),
                    limits = c(0.1, .75)) +
...
#| fig-cap: "Sampling distributions for population parameter estimates displayed separately by selection method and population parameter. Selection methods that do not control for X are displayed as dashed lines."
#| fig-height: 8
#| code-fold: true
