...
We can look at the interaction between the number of covariates and the proportion of good covariates by calculating the number of good covariates: $n\_good\_covs = n\_covs * p\_good\_covs$. This represents the number of covariates that have a nonzero relationship with $Y$.

```{r}
#| label: tbl-typeI-ngoodcovs
...
d_0 |> 
  ...
  summarise(prop_sig = mean(p_value < 0.05),
            .groups = "drop") |> 
  group_by(n_good_covs, method) |> 
  summarise(typeII_min = min(prop_sig),
            typeII_max = max(prop_sig),
            typeII_mean = mean(prop_sig),
            .groups = "drop") |> 
  kbl(caption = "Type I error by n_good_covs")

```

```{r}
#| label: fig-typeI-ngoodcovs 
...
d_0 |> 
  ...
  summarise(prop_sig = mean(p_value < 0.05),
            .groups = "drop") |> 
  ggplot(aes(x = n_good_covs, y = prop_sig, color = method, linetype = method)) + 
  geom_line() +
  geom_hline(yintercept = 0.05, linetype = "dotted", color = "black") +
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 8, 9, 10, 12, 15)) +
  scale_y_continuous(breaks = c(0.05, 0.10, 0.15, 0.20, 0.25, 0.30)) +
  labs(y = "Type I error",
       title = "Type I error by n_good_covs") +
  scale_linetype_manual(values = method_linetypes) +
  scale_color_manual(values = method_colors)

```

We see the non-monotonic, but increasing trend for the p-hacked line. As the number of good covariates increases, lasso and full lm show inflation of Type I error, while the other methods remain at or around 0.05.
...
d_03 |> 
  ...
  summarise(prop_not_sig = mean(p_value >= 0.05),
            .groups = "drop") |> 
  ggplot(aes(x = n_good_covs, y = prop_not_sig, color = method, linetype = method)) + 
  geom_line() +
  scale_x_continuous(breaks = c(1, 2, 3, 4, 5, 6, 8, 9, 10, 12, 15)) +
  labs(y = "Type II error",
...
In the plot, we see the overall decreasing trend in Type II error (except for no covariates). Including all covariates yields higher Type II error than selecting them, although full lm performs worse than this for higher numbers of good covariates. Lasso has the lowest Type II error as the number of good covariates increases.

##### b_x = 0.5

```{r}
#| label: tbl-typeII-ngoodcovs-05
...
Similarly, we see lasso performing best for higher numbers of good covariates. Full lm and all covariates become comparable as the number of good covariates increases.
