...
########################################################
ver = 8                    #for output file names to track versions
...
for(AFile in dFiles$Filenames){
  if(!file.exists(file.path(OutPath, AFile))){
    ...
    iF = which(AFile %in% dFiles$Filenames)
    Bx = dFiles$Bxs[iF]
    N = dFiles$Ns[iF]
    Kc = dFiles$Kcs[iF]
    ...
    #The simulations          
    for (s in (1:S)){
      if (s %% 10 == 0) {message('...Simulation #: ',s)}  #used to confirm simulation is progressing
