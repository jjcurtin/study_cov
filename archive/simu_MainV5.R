...
rMin = -.5                 #minimum r between Covs
rMax = .5                  #maximum r between Covs
rSD = .1                   #SD of normal distribution of rs among Covs (MEAN =0)
ver = 5                    #for output file names to track versions
    ...
    for (Kc in Kcs){
      for (dR2 in dR2s){
        for (nSig in nSigs){
          ...
          #This should maintain sd(Y) at 1 + the sd induced by manipulation of X (with SD 1 in each level of X)
          ...
          for (s in (1:S)){
            ...
            Covars = rtruncnorm(nCovars,rMin, rMax,0,rSD) #get truncated normal distribution of covariances
