...
dFiles = expand.grid(Ns,Bxs,Kcs,rCYMeans,nSigs)
names(dFiles) = c('Ns', 'Bxs', 'Kcs', 'rCYMeans', 'nSigs')
dFiles$nSigsS = varRecode(dFiles$nSigs, nSigs, c('Q', 'H', 'T'))
dFiles$Success = NA

CovModel = str_c('rCY', dFiles$rCYMeans*100, dFiles$nSigsS, dFiles$Kcs)
dFiles$Filenames = str_c('v', ver, '_Bx', dFiles$Bxs*10, '_', CovModel, '_', dFiles$Ns, '.rds')

#reverse order
Index = 1:nrow(dFiles)
#dFiles = dFiles[order(Index, decreasing=TRUE),]
########################################################
OutPath = str_c('P:/StudyData/COV/Analysis/Data/', 'v', ver)
for(AFile in dFiles$Filenames){
  if(!file.exists(file.path(OutPath, AFile))){
    iF = which(AFile == dFiles$Filenames)
    Bx = dFiles$Bxs[iF]
    N = dFiles$Ns[iF]
    Kc = dFiles$Kcs[iF]
    rCYMean = dFiles$rCYMeans[iF]
    nSig = dFiles$nSigs[iF]
          
    #report simulation characteristics
    message(sprintf('Simulating: Bx= %0.1f; N= %0.0f; Kc= %0.0f; rCY= %0.2f; nSig= %0.2f', Bx, N, Kc, rCYMean, nSig))
    ...
    #The simulations          
    for (s in (1:S)){
      if (s %% 10000 == 0) {message('...Simulation #: ',s)}  #used to confirm simulation is progressing
      ...
      while(!IsPD) {
        ...
        if(!IsPD && iCnt>10000) break
      }
      
      if(IsPD){
        dFiles$Success[iF] = TRUE
        ...
        dFiles$Success[iF] = FALSE
        message(str_c('   FAILED TO FIND PD SIGMA: ', AFile))
      }
    ...
    if(IsPD){
      ...
      saveRDS(d,file=file.path(OutPath,AFile))
    }
  } #if exists
} #for filenames
saveRDS(dFiles,file.path(OutPath, 'ForwardOrder.rds'))
###################################
